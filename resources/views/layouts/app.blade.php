...
<body class="font-sans antialiased bg-slate-50 text-slate-900" x-data="{ sidebarOpen: false }">
    <div class="min-h-screen flex flex-col md:flex-row">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="p-5">
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 border-b border-slate-800 pb-5 mb-6">
                    <div
                        class="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-bold text-lg shadow-md">
                        E
                    </div>
                    <div class="flex flex-col">
                        <span class="text-lg font-bold tracking-tight text-white leading-none">EcoFert</span>
                        <span class="text-xs text-emerald-400 font-medium leading-tight mt-0.5">Decision Support
                            System</span>
                    </div>
                </a>
                ...
                <nav class="space-y-1.5 text-sm">
                    ...
                    @if(Auth::user()->isHousehold())
                        <span class="px-3 text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Household
                            Portal</span>
                        ...
                        <a href="{{ route('household.dashboard') }}"
                            ...
                            <svg class="w-5 h-5 opacity-75" fill="none" stroke="currentColor" stroke-width="1.75"
                                viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('household.experiments.index') }}"
                            ...
                            <svg class="w-5 h-5 opacity-75" fill="none" stroke="currentColor" stroke-width="1.75"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 18v-5.25m0 0a6.01 6.01 0 0 0 1.5-.189m-1.5.189a6.01 6.01 0 0 1-1.5-.189m3.75 7.478a12.06 12.06 0 0 1-4.5 0m3.75 2.383a14.406 14.406 0 0 1-3 0M14.25 18v-4.86c0-.98-.626-1.847-1.564-2.146L12 10.75l-.686.244c-.938.299-1.564 1.166-1.564 2.146V18" />
                            </svg>
                        ...
                        <a href="{{ route('household.chat.history') }}"
                            ...
                            <svg class="w-5 h-5 opacity-75" fill="none" stroke="currentColor" stroke-width="1.75"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a.75.75 0 0 1-.816-.937 4.5 4.5 0 0 0 .445-1.921A7.682 7.682 0 0 1 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                            </svg>
                    ...
                    @if(Auth::user()->isExtensionOfficer())
                        <span class="px-3 text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Extension
                            Workspace</span>
                    ...
                    @if(Auth::user()->isAdmin())
                        <span class="px-3 text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">System
                            Administration</span>
            ...
            <div class="p-4 border-t border-slate-800 bg-slate-950/40">
                <div class="flex items-center justify-between">
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 group">
                        ...
                        @else
                            <div
                                class="w-9 h-9 rounded-full bg-emerald-700 flex items-center justify-center text-xs font-bold text-white font-mono shadow border border-emerald-600">
                                {{ Auth::user()->initials }}
                            </div>
                        @endif
                        <div class="flex flex-col">
                            <span
                                class="text-xs font-bold text-white group-hover:text-emerald-400 transition leading-tight">{{ Auth::user()->name }}</span>
                            <span
                                class="text-[10px] text-slate-400 font-mono leading-tight mt-0.5">{{ Auth::user()->role_type }}</span>
                        </div>
                    </a>
        ...
        <div class="flex-1 flex flex-col md:pl-64 min-h-screen">
            ...
            <!-- Top Header Bar -->
            <header
                class="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8">

                <div class="flex items-center gap-3">
                    <!-- Mobile Hamburger Button -->
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="p-2 rounded-lg text-slate-600 hover:bg-slate-100 md:hidden focus:outline-none">
                    ...
                    <div class="flex items-center gap-2">
                        <h1 class="text-base font-bold text-slate-900 tracking-tight">EcoFert Decision Support System
                        </h1>
                        <span
                            class="hidden sm:inline-block px-2.5 py-0.5 text-xs font-mono font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full">
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                ...
                                @else
                                    <div
                                        class="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center text-xs font-bold text-white font-mono shadow-sm">
                                        {{ Auth::user()->initials }}
                                    </div>
                                @endif
                                <div class="hidden sm:flex flex-col text-left">
                                    <span
                                        class="text-xs font-bold text-slate-900 leading-tight">{{ Auth::user()->name }}</span>
                                    <span
                                        class="text-[10px] text-emerald-700 font-semibold font-mono leading-tight">{{ Auth::user()->role_type }}</span>
                                </div>
